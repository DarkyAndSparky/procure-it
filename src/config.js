...
// По умолчанию приложение доступно только на этом компьютере. Для LAN
// задайте PROCURE_BIND_HOST=0.0.0.0 явно.
const BIND_HOST  = process.env.PROCURE_BIND_HOST || '127.0.0.1';
const ROOT_DIR   = path.join(__dirname, '..');
// PROCURE_DATA_DIR: только для тестов (см. e2e/) — позволяет поднять сервер
// на изолированной БД/файлах вместо настоящих data/ репозитория. В обычном
// запуске (start.bat/Docker/systemd) переменная не задана, поведение не
// меняется ни на бит.
const DATA_DIR   = process.env.PROCURE_DATA_DIR
  ? path.resolve(process.env.PROCURE_DATA_DIR)
...
const DEFAULT_SETTINGS = {
  ...
  smtpHost:    '',   // SMTP-хост, напр. smtp.yandex.ru, smtp.gmail.com, 192.168.1.10
  smtpPort:    '587', // 587 = STARTTLS (рекомендуется), 465 = SSL/TLS, 25 = plain
  smtpSecure:  '0',  // '1' = SSL/TLS на порту 465; '0' = STARTTLS (587) или plain
  smtpUser:    '',   // Логин (обычно email-адрес)
  smtpPass:    '',   // Пароль или app-пароль
  smtpFrom:    '',   // Адрес отправителя; если пусто — используется smtpUser
};
